...
public class ToDosController {
  ...
  @POST
  @Consumes(MediaType.APPLICATION_FORM_URLENCODED)
  public String saveToDo(@PathParam("documentId") String documentId,
      @FormParam("title") @NotEmpty String title, @FormParam("status") String status) {
    ToDo todo = repository.findById(documentId)
        .orElseThrow(() -> new NotFoundException());
    todo.setTitle(title);
    todo.setStatus(
        status == null || status.isEmpty() ? ToDo.State.Incomplete : ToDo.State.valueOf(status));
    repository.save(todo);
